...
const ThirdPlace = () => {
  ...
  const handleUpdateScore = async () => {
    if (currentGame) {
      ...
      if (error) {
        ...
        getGamesByRoundId(8); // Refresh the games data
      }
    }
  }
  ...
  const handleResetAllGames = async () => {
    Alert.alert(
      "Reset All Games",
      "Are you sure you want to reset all scores to 0 and mark all games as unfinished for 3rd Place Games?",
      [
        {
          text: "Cancel",
          style: "cancel"
        },
        {
          ...
          onPress: async () => {
            const { error } = await supabase
              .rpc('reset_bracket_scores', { round_id_param: 8 });
  
            if (error) {
              console.error('Error resetting games:', error);
              Alert.alert('Error', 'Failed to reset games');
            } else {
              Alert.alert('Success', 'All 3rd Place Games have been reset');
              getGamesByRoundId(8);
            }
          }
        }
  ...
  const renderPlaceholder = () => (
    <View style={styles.placeholderContainer}>
      <Text style={styles.placeholderText}>
        3rd Place games will be available after Semi Finals are completed.
      </Text>
      <Text style={styles.placeholderSubText}>
        Please check back later when all Semi Final games are finished.
      </Text>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <Text className='font-outfitbold text-2xl text-center m-5'>3rd Place</Text>
      {isLoading ? (
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>3rd Place</Text>
            {currentGame && (
              <>
